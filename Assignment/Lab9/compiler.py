...
class Scanner:
    ...
    def __init__(self, input_file):
        ...
        self.input_string = input_file.read()              #Do not forget to change back 
        
        # Used to help debug and visulize what i am doing 
        # file_path = r"C:\Users\tomar\OneDrive\Documents\UC\24sem1\Cosc261\Assignment\Lab8\program"
    ...
    def get_token(self):
        ...
        '''
        # To catch the end of the input_string being processed by skip_white_space()
        if self.current_char_index >= len(self.input_string):
            return (None, '')
    ...
    def consume(self, *expected_tokens):
        ...
        if self.next_token[0] in expected_tokens:
            ...
            self.next_token = self.get_token()
            if token[0] in [Token.NUM, Token.ID]:
...
class If_Else_AST:
    def __init__(self, condition, then, else1):
        self.condition = condition
        self.then = then 
        self.else1 = else1
    def __repr__(self):
        return 'if else' + repr(self.condition) + ' then ' + \
                       repr(self.then) + \
                       ' else ' + repr(self.else1) + ' end'
    def indented(self, level):
        return indent('If-Else', level) + \
               self.condition.indented(level+1) + \
               self.then.indented(level+1) + \
               self.else1.indented(level+1)
    def code(self):
        l1 = label_generator.next()
...
def if_statement():
    ...
    if scanner.lookahead() == Token.ELSE:
        ...
        return If_Else_AST(condition, then, else1)
    scanner.consume(Token.END)
